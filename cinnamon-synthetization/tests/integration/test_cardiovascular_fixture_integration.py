...
def test_cardiovascular_fixture_files_exist():
    assert DATA_PATH.exists(), f"Dataset not found: {DATA_PATH}"
    assert CONFIG_PATH.exists(), f"Config not found: {CONFIG_PATH}"


def test_cardiovascular_row_count_matches_sampling_num_samples():
...
def test_cardiovascular_attribute_count_and_columns_match_config():
    df = pd.read_csv(DATA_PATH)
    config = _load_config()
