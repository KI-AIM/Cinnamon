...
STATUS_GLOB = "it-obesity-levels-*.yaml"
...
class MockObesityLevelsSynthesizer(TabularDataSynthesizer):
    def __init__(self):
...
def _cleanup_obesity_status_files():
...
def cleanup_status_files():
    _cleanup_obesity_status_files()
    yield
    _cleanup_obesity_status_files()


def test_obesity_levels_api_generates_synthetic_dataset(monkeypatch):
    ...
    session_key = f"it-obesity-levels-{uuid.uuid4().hex}"
    ...
    with CONFIG_PATH.open("rb") as attribute_config, CONFIG_PATH.open("rb") as algorithm_config, DATA_PATH.open("rb") as data_file:
        response = client.post(
            "/start_synthetization_process/obesity_levels_mock",
            data={
