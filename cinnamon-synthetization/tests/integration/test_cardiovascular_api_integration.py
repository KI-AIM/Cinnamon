...
# Avoid importing heavyweight production synthesizer registry in this test module.
...
STATUS_GLOB = "it-cardio-*.yaml"


class InlineProcess:
    """Runs multiprocessing targets inline to keep tests deterministic and fast."""
    ...
    def start(self):
        # Emulate multiprocessing semantics: target exceptions should not crash caller.
        try:
            self.target(*self.args, **self.kwargs)
        except Exception as exc:  # pragma: no cover - defensive path for test stability
            self.error = exc
...
class MockCardiovascularSynthesizer(TabularDataSynthesizer):
    def __init__(self):
...
def cleanup_status_files():
    _cleanup_cardio_status_files()
    yield
    _cleanup_cardio_status_files()


def test_cardiovascular_api_generates_synthetic_dataset(monkeypatch):
    ...
    session_key = f"it-cardio-{uuid.uuid4().hex}"
    ...
    with CONFIG_PATH.open("rb") as attribute_config, CONFIG_PATH.open("rb") as algorithm_config, DATA_PATH.open("rb") as data_file:
        response = client.post(
            "/start_synthetization_process/cardiovascular_mock",
            data={
    ...
    assert isinstance(process_obj, InlineProcess), (
        f"Expected InlineProcess, got {type(process_obj)!r}. "
        "Process patching did not apply."
    )
