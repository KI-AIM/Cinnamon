...
class MockStrokeSynthesizer(TabularDataSynthesizer):
    def __init__(self):
...
def _cleanup_stroke_status_files():
...
def cleanup_status_files():
    _cleanup_stroke_status_files()
    yield
    _cleanup_stroke_status_files()


def test_stroke_api_generates_synthetic_dataset(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        "stroke_mock",
        {
            ...
            "class": MockStrokeSynthesizer,
            "display_name": "Stroke Mock",
    ...
    session_key = f"it-stroke-{uuid.uuid4().hex}"
    ...
    with CONFIG_PATH.open("rb") as attribute_config, CONFIG_PATH.open("rb") as algorithm_config, DATA_PATH.open("rb") as data_file:
        response = client.post(
            "/start_synthetization_process/stroke_mock",
            data={
