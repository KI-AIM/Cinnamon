...
STATUS_GLOB = "it-pima-diabetes-*.yaml"
...
class MockPimaDiabetesSynthesizer(TabularDataSynthesizer):
    def __init__(self):
...
def _cleanup_pima_status_files():
...
def cleanup_status_files():
    _cleanup_pima_status_files()
    yield
    _cleanup_pima_status_files()


def test_pima_diabetes_api_generates_synthetic_dataset(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        "pima_diabetes_mock",
        {
            ...
            "class": MockPimaDiabetesSynthesizer,
            "display_name": "Pima Diabetes Mock",
    ...
    session_key = f"it-pima-diabetes-{uuid.uuid4().hex}"
    ...
    with (
        ...
        response = client.post(
            "/start_synthetization_process/pima_diabetes_mock",
            data={
    ...
    assert status["components"]["total_synthesis"]["completed"] in {True, "True"}
    assert status["components"]["total_synthesis"]["duration"] != "Waiting"
